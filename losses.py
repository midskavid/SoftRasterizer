...
class P2MLoss(nn.Module):
    ...
    def laplace_regularization(self, input1, input2, block_idx):
        ...
        # dims = tup        lap1 = self.laplace_coord(input1, self.laplace_idx[block_idx])
        lap2 = self.laplace_coord(input2, self.laplace_idx[block_idx])
        ...
        return laplace_loss, move_loss, l2
    ...
    def forward(self, outputs):
        ...
        for i in range(3):
            ...
            lap, move, l2C = self.laplace_regularization(pred_coord_before_deform[i],
                                                                   pred_coord[i], i)
            lap_loss += lap_const[i] * lap + l2_const[i]*l2C
