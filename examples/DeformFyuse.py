...
def main():
    ...
    renderer = sr.SoftRenderer(image_size=256, sigma_val=1e-4, aggr_func_rgb='hard', 
                               camera_mode='projection', viewing_angle=42.5)
    ...
    for ii in range(numFrames) :
        idx = poseData.keys()[ii]
        projMat = torch.mm(poseData.poses[ii]['K'],poseData.poses[ii]['Rt'][0:3,:])
        imgGt = imageio.read(args.data_dir+'{0:09d}'.format(idx))
        # swap axes ??
    ...
    images_gt = torch.from_numpy(imagesGT).cuda()
    projMatrices = torch.tensor(projMatrices).cuda()
